...
# This script helps set up bytedrop on a fresh Ubuntu/Debian VPS
...
# git clone https://github.com/yourusername/bytedrop.git
# cd bytedrop
...
# Create the bytedrop configuration file with the correct content
echo "Creating /etc/nginx/sites-available/bytedrop..."
cat <<EOF | sudo tee /etc/nginx/sites-available/bytedrop
server {
    listen 80;
...
# Create the symbolic link to enable the bytedrop site
sudo ln -sf /etc/nginx/sites-available/bytedrop /etc/nginx/sites-enabled/bytedrop
...
if [ $? -eq 0 ]; then
    ...
    echo "Nginx configuration test failed. Please check /etc/nginx/nginx.conf and /etc/nginx/sites-available/bytedrop."
    exit 1
fi
...
cd ui
pm2 start npm --name bytedrop-frontend -- start
cd ..
...
echo "=== Setup Complete ==="
echo "bytedrop is now running on your VPS!"
